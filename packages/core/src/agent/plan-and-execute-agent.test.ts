...
describe('PlanAndExecuteAgent', () => {
  it('completes full planning → approval → execution flow', async () => {
    ...
    expect(toolRequests).toContainEqual(
  ...
  it('parses plan with numbered list format', async () => {
    ...
    const provider = mockProvider([
      ...
      // Execution steps
      [{ type: 'text', text: 'Done 1' }, { type: 'finish', reason: 'stop' }],
      [{ type: 'text', text: 'Done 2' }, { type: 'finish', reason: 'stop' }],
      [{ type: 'text', text: 'Done 3' }, { type: 'finish', reason: 'stop' }],
    ]);
  ...
  it('treats unstructured text as single step', async () => {
    ...
    const provider = mockProvider([
      ...
      [{ type: 'text', text: 'All done.' }, { type: 'finish', reason: 'stop' }],
    ]);
    ...
    if (planCreated?.type === 'plan_created') {
      ...
      expect(planCreated.plan.steps[0].description).toBe(
