...
import type { ContentPart, Message, ToolResultPart } from '../llm/types.js';
...
export abstract class BaseAgent {
    ...
    async *run(input: string | Message[], signal?: AbortSignal): AsyncGenerator<AgentEvent> {
        ...
        // 支持同一 Agent 实例多次调用：重置状态机、循环检测器和待处理审批
    ...
    protected async *executeToolCalls(
        ...
        for (const tc of toolCalls) {
            ...
            if (decision.allow) {
                ...
                rejectedCalls.set(
                    tc.id,
                    `Tool call "${tc.name}" was rejected by onBeforeToolCall hook`,
                );
            }
        }
