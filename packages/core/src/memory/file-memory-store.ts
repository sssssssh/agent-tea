...
export class FileMemoryStore implements MemoryStore {
    ...
    constructor(private readonly baseDir: string = '.agent-tea/memory') {
        this.entriesDir = path.join(baseDir, 'entries');
        this.indexPath = path.join(baseDir, 'index.json');
    }

    async set(key: string, content: string, tags?: string[]): Promise<void> {
        ...
        await fs.writeFile(this.getEntryPath(key), JSON.stringify(entry, null, 2), 'utf-8');
    ...
    async search(tags?: string[]): Promise<MemoryEntry[]> {
        ...
        return all.filter((entry) => tags.some((tag) => entry.tags?.includes(tag)));
    }
