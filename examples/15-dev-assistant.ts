...
const codeSearcher = subAgent({
  name: 'search_code',
  description: '在代码库中搜索特定模式、关键词或代码结构。返回搜索结果的摘要。适合需要跨多个文件查找信息的任务。',
  provider,
  model,
...
async function consumeEvents(events: AsyncGenerator<AgentEvent>) {
  for await (const event of events) {
    switch (event.type) {
      ...
      case 'tool_request':
        console.log(`  -> ${event.toolName}(${JSON.stringify(event.args).slice(0, 100)}${JSON.stringify(event.args).length > 100 ? '...' : ''})`);
        break;
      ...
      case 'approval_request':
        ...
        const approved = approveAnswer === '' || approveAnswer.toLowerCase().startsWith('y');
