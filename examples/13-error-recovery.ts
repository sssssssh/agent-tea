...
async function scenario1_providerRetry() {
    ...
    try {
        ...
        if (error instanceof ProviderError) {
            console.log(
                `  [立即失败] statusCode=${error.statusCode}, retryable=${error.retryable}`,
            );
...
async function scenario2_maxIterations() {
    ...
    for await (const event of agent.run(query)) {
        switch (event.type) {
            case 'message':
                console.log(
                    `  [Assistant] ${event.content.slice(0, 100)}${event.content.length > 100 ? '...' : ''}`,
                );
                break;
    ...
    const unstableApi = tool(
        {
            ...
            description:
                '调用外部 API 获取数据。注意：这个 API 不太稳定，可能会失败。失败时可以换个 endpoint 重试。',
            parameters: z.object({
        ...
        async ({ endpoint }) => {
            ...
            if (endpoint.includes('users')) {
                ...
                throw new Error(
                    'Connection timeout: external API at /users is not responding (ETIMEDOUT)',
                );
            }
