...
const knowledgeBase: Record<string, string> = {
    ...
    typescript:
        'TypeScript 是 JavaScript 的超集，添加了静态类型系统。广泛用于大型项目，提供更好的开发体验和代码质量。',
...
const researcher = subAgent({
    name: 'research',
    description:
        '技术研究员：负责调研技术方案、查阅知识库、对比分析。给它一个研究任务，它会返回详细的调研报告。',
    provider,
    model,
    tools: [lookupKB, compareTech],
    systemPrompt:
        '你是一个技术研究员。使用知识库工具查找信息，必要时进行技术对比。输出结构化的研究报告。用中文回答。',
...
const coder = subAgent({
    name: 'write_code',
    description:
        '编码员：负责根据需求编写代码。给它具体的编码任务，它会返回可运行的代码片段和说明。',
    provider,
    model,
    tools: [generateCode, explainCode],
    systemPrompt:
        '你是一个 TypeScript 编码专家。根据需求生成高质量代码，并解释实现思路。用中文回答。',
    maxIterations: 8,
});
