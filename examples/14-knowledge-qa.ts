...
const remember = tool(
    {
        ...
        parameters: z.object({
            key: z
                .string()
                .describe('知识点的唯一标识，如 "project-architecture" 或 "api-design-pattern"'),
            content: z.string().describe('要记住的知识内容，尽量精炼准确'),
            tags: z
                .array(z.string())
                .optional()
                .describe('分类标签，如 ["architecture", "design"]，便于后续搜索'),
        }),
...
const recall = tool(
    {
        ...
        parameters: z.object({
            tags: z
                .array(z.string())
                .optional()
                .describe('按标签搜索，如 ["architecture"]。不传则列出所有记忆。'),
        }),
        tags: ['readonly'],
    },
    async ({ tags }) => {
...
async function consumeEvents(events: AsyncGenerator<AgentEvent>) {
    for await (const event of events) {
        switch (event.type) {
            ...
            case 'tool_request':
                console.log(
                    `  -> ${event.toolName}(${JSON.stringify(event.args).slice(0, 120)}${JSON.stringify(event.args).length > 120 ? '...' : ''})`,
                );
                break;
