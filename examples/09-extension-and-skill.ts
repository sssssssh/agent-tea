...
import { Agent, extension, skill, tool, z, builtinTools } from '../packages/sdk/src/index.js';
...
const weatherExt = extension({
    ...
    instructions:
        '当用户询问天气时，使用天气工具查询。温度统一使用摄氏度。支持的城市：北京、上海、深圳、杭州、成都。',
...
const measureComplexity = tool(
    ...
    async ({ code }) => {
        ...
        const maxNesting = Math.min((code.match(/{/g) || []).length, 5);
...
const agent = new Agent({
    ...
    tools: [...(weatherExt.tools || []), ...(reviewSkill.tools || [])],
...
async function main() {
    ...
    console.log(
        `内置工具扩展 builtinTools 包含: ${builtinTools.tools?.map((t) => t.name).join(', ')}`,
    );
