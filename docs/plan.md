# Agent-Tea Server — 设计方案

## 目标

基于 agent-tea 框架构建中心化 Agent 服务平台，通过 HTTP API 让公司各团队（技术栈不统一）快速获得 AI 能力。
...
| 工具来源 | 团队注册自己的业务 HTTP API，平台自动包装成 agent-tea Tool |
...
- 平台自动转为 agent-tea Tool（HTTP Tool Bridge）
...
```
agent-tea-server (NestJS)
  ├── @agent-tea/core       — Agent 引擎
  ├── @agent-tea/sdk        — Extension/Skill/SubAgent
  ├── @agent-tea/provider-* — LLM 适配器（按需选用）
  └── @nestjs/*             — HTTP 框架
```

## 核心总结

NestJS + agent-tea，6 个模块，两种入口，先做日志分析标杆。
